...
export class hourEvents {
  constructor(public hour: string, public events?: Event[]) {}
}
...
export class DayComponent implements OnInit {
  ...
  public initDay() {
    ...
    let eventOfDay = this.allEvents.filter(
      item => item.date === this.selectDay
    );
    ...
    this.currentDay = this.selectDay
      .split("-")
      .reverse()
      .join(".");
  }
  ...
  public async changeDay(diff: number) {
    const selectedDay: string[] = this.activateRoute.snapshot.params[
      "date"
    ].split("-");
    const newDate = new Date(
      parseInt(selectedDay[0]),
      parseInt(selectedDay[1]) - 1,
      parseInt(selectedDay[2]) + diff
    );
