...
export class EventsDialogComponent implements OnInit {
  ...
  onAddClick(): void {
    this.error = false;
    if((this.title.length > 1 || this.title.length < 11) && this.type != "" && this.date != "" && this.time != ""){
      ...
      let updatedDate: string = `${newDate.getFullYear()}-${newMonth}-${newDay}`;

      if(this.data.date == updatedDate && this.data.time == this.time){
        this.eventDataService.removeEvent(this.data.date, this.data.time)
        this.eventDataService.addEvent(updatedDate, this.time, this.title, this.description, this.type);
        this.dialogRef.close();
      }
      else{
        if(!this.events.find(item => ((item.date == updatedDate) && (item.time == this.time)))){
